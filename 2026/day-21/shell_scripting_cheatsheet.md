...
- **Example**:
    ```bash
    echo $NAME # Prints Mahesh
    ```
- **Quoting**:
 - **Double quotes " "**: Variable is replaced with its value.
    ```bash
    echo "Hello $NAME"   # Prints Hello Mahesh
    ```
...
- $?: Last command status
 ...
 ```bash
    ...
    ls /mahesh                              # This directory does not exist
    ...
    ./script.sh Mahesh                      # $1 = Mahesh, $# = 1, $@ = Mahesh
...
- **Syntax**:
    ...
    ```
- **Example**: Prints user IDs for root,ubuntu,and mahesh
    ```bash
    for USER in root ubuntu mahesh; do
        id "$USER"
    done
    ```
